...
def home():
   ...
   .btn{{background:#9147ff;color:white;padding:15px 30px;border-radius:10px;text-decoration:none;display:inline-block}}
    ...
    <p>Bot ONLINE 24/7</p>
    <div class="card"><h2>RAIDS BLOQUEADAS</h2><div class="num">{stats["raids"]}</div></div>
    <div class="card"><h2>MENSAJES ANALIZADOS</h2><div class="num">{stats["msgs"]}</div></div>
    <div class="card"><h2>NUKES BLOQUEADOS</h2><div class="num">{stats["nukes"]}</div></div>
    <div class="card"><h2>WEBHOOKS BORRADOS</h2><div class="num">{stats["webhooks"]}</div></div>
    <div class="card"><a href="{STRIPE_LINK}" class="btn">COMPRAR PRO - 4.99€</a><br><br>Comando:!liminal</div>
    </body></html>
    """
    return html
...
async def liminal(ctx):
    embed = discord.Embed(title="🛡️ LIMINAL SHIELD PRO", color=0x9147ff, description=f"Web: {os.getenv('RENDER_EXTERNAL_URL','online')} | [Comprar PRO]({STRIPE_LINK})")
